...
def test_logout(logged_in_page: DashboardPage):
    with allure.step("Открыть дашборд и выполнить выход с помощью метода logout()"):
        auth_page = logged_in_page.logout()
    with allure.step("Проверить загрузку страницы авторизации по видимости поля email"):
